...
# Worked example 6.3
list.files()
smoke <- readRDS("mod06_smoking_status.rds")

# z test
x <- sum(smoke$smoking_status == "Smokers")
n <- nrow(smoke)

z.test <- prop.test(x=x, n=n, p=0.2, alternative = "two.sided", conf.level=0.95)
z.test

# binom test 
b.test <- binom.test(x = x, n = n, p = 0.20)
b.test

# Worked example 6.4 ----------------------------------------------
# data 
se <- readRDS("mod06_nausea.rds")
